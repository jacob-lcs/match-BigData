...
if __name__ == "__main__":
    ...
    total = 0
    addition2 = {'addition': '全国', '技术文员': 0, '计算机辅助设计工程师': 0, '高级软件工程师': 0, 'ERP技术开发': 0, 'ERP实施顾问': 0,
    ...
    for key in infos:
        ...
        addition2[position] += salary
        p = position + "1"
        addition2[p] += 1
    # print(cs2)
    for key in addition2:
        if key != 'addition':
            if '1' not in key:
                if addition2[key] != 0:
                    key1 = key + '1'
                    addition2[key] = addition2[key] / addition2[key1]

    newTable = "全国平均工资统计.xls"  # 表格名称
    wb = xlwt.Workbook(encoding='utf-8')  # 创建excel文件，声明编码
    ws = wb.add_sheet('全国平均工资统计')  # 创建表格
